...
const ZAИКА_MEMORY_KEY="blueprint.zaika.memory";
const MAX_ZAИКА_MEMORY=20000;
...
function linkedText(value){
  ...
  while((match=pattern.exec(text))){
    ...
    if(!match[2]){
      ...
      if(tail){trailing=tail;url=url.slice(0,-tail.length)}
    }
    const label=match[1]||url;
    nodes.push(<a key={`link-${index++}`} href={url} target="_blank" rel="noopener noreferrer" title={url} style={{color:C.hi,textDecoration:"underline",textUnderlineOffset:2}}>{label}</a>);
...
export default function BlueprintPage(){
  ...
  useEffect(()=>{setAccessKey(sessionStorage.getItem("blueprint.key")||"");setZaikaMemory(localStorage.getItem(ZAИКА_MEMORY_KEY)||"");const saved=sessionStorage.getItem("blueprint.provider");if(saved==="local"||saved==="cloud"||saved==="zaika")setProvider(saved);const fn=e=>{if(e.key==="Escape")setMasked(v=>!v)};window.addEventListener("keydown",fn);return()=>window.removeEventListener("keydown",fn)},[]);
  ...
  function manageZaikaMemory(){
    const next=window.prompt("Paste your ChatGPT Memory Summary for ЗАИКА. It will be stored in this browser's localStorage and sent to the Zaika backend only when Zaika is selected. Submit an empty value to clear it.",zaikaMemory);
    if(next===null)return;
    const clean=next.trim();
    if(clean.length>MAX_ZAИКА_MEMORY){window.alert(`Zaika memory is too large. Maximum ${MAX_ZAИКА_MEMORY} characters.`);return;}
    if(clean){localStorage.setItem(ZAИКА_MEMORY_KEY,clean);setZaikaMemory(clean);setEntries(v=>[...v,{id:Date.now(),kind:"SYSTEM RESULT · ЗАИКА",text:`MEMORY IMPORTED\n${clean.length} CHARACTERS\nSOURCE: USER-PASTED CHATGPT MEMORY SUMMARY\nSTORAGE: THIS BROWSER`,user:false,error:false,previews:[]}])}
    else{localStorage.removeItem(ZAИКА_MEMORY_KEY);setZaikaMemory("");setEntries(v=>[...v,{id:Date.now(),kind:"SYSTEM RESULT · ЗАИКА",text:"MEMORY CLEARED FROM THIS BROWSER",user:false,error:false,previews:[]}])}
  }
  ...
  async function run(){
    ...
    if(!invalid){
      try{
        const response=await fetch(endpoint,{method:"POST",headers:{"content-type":"application/json",...(accessKey?{"x-blueprint-key":accessKey}:{})},body:JSON.stringify({raw,plan:plan.map(s=>({id:s.id,type:s.type,body:s.body,normalized:normalize(s.body)})),...(provider==="zaika"&&zaikaMemory?{memory:zaikaMemory}:{})})});
  ...
  const metaColor=v=>v==="CONNECTED"||v==="SET"||v==="KEY SET"||v==="IMPORTED"?C.ok:v==="LOCAL READ ONLY"||v==="CHAT ONLY"?accent:(String(v).includes("REQUIRED")||String(v).includes("MISSING")||v==="DISCONNECTED"||v==="SERVER SETUP"||v==="EMPTY")?C.danger:C.text;
