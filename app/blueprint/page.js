...
export default function BlueprintPage(){
  ...
  const [entries,setEntries]=useState([{id:1,kind:"SYSTEM RESULT",text:'CONSOLE READY\nMODE: DUAL BRAIN\nCHAIN PARSER: ACTIVE\nNESTED ARGUMENTS: ACTIVE\n\nCLOUD: GROQ / gpt-oss-120b\nLOCAL: OLLAMA / blueprint-local\n\nLOCAL currently handles reflect(...) reasoning.\nCLOUD keeps repository tools and deployment actions.\n\nSet AGENT KEY, select a brain, then run a command.',user:false,error:false,previews:[]}]);
  const logRef=useRef(null);

  useEffect(()=>{setAccessKey(sessionStorage.getItem("blueprint.key")||"");const saved=sessionStorage.getItem("blueprint.provider");if(saved==="local"||saved==="cloud")setProvider(saved);const fn=e=>{if(e.key==="Escape")setMasked(v=>!v)};window.addEventListener("keydown",fn);return()=>window.removeEventListener("keydown",fn)},[]);
  ...
  const providerLabel=provider==="local"?"OLLAMA LOCAL":"GROQ CLOUD";
  const providerShort=provider==="local"?"OLLAMA":"GROQ";
  const modelLabel=provider==="local"?"blueprint-local":"gpt-oss-120b";

  function chooseProvider(next){
    setProvider(next);setAgent("DISCONNECTED");sessionStorage.setItem("blueprint.provider",next);
    setEntries(v=>[...v,{id:Date.now(),kind:"SYSTEM RESULT",text:next==="local"?"LOCAL BRAIN SELECTED\nROUTE: /api/command/local → secure bridge → Ollama\nMODEL: blueprint-local\nMODE: REFLECT / REASONING":"CLOUD BRAIN SELECTED\nROUTE: /api/command → Groq\nMODEL: gpt-oss-120b\nMODE: FULL REPO TOOLS + DEPLOY",user:false,error:false,previews:[]}]);
  }
  ...
  async function run(){
    ...
    if(!invalid){
      try{
        ...
        else{if(response.status===401)setAgent("AUTH REQUIRED");else if(response.status===409&&provider==="local")setAgent("LOCAL REASON ONLY");else if(response.status===503)setAgent("SERVER SETUP");else setAgent("DISCONNECTED");output=data?.output||formatPlan(raw,plan);error=true}
  ...
  const modeBtn=active=>({...btn,borderColor:active?(provider==="local"?C.local:C.ok):C.line,background:active?"rgba(70,120,145,.22)":"transparent",color:active?(provider==="local"?C.local:C.ok):C.text});
  const metaColor=v=>v==="CONNECTED"||v==="SET"||v==="KEY SET"?C.ok:v==="LOCAL REASON ONLY"?C.local:(String(v).includes("REQUIRED")||String(v).includes("MISSING")||v==="DISCONNECTED"||v==="SERVER SETUP")?C.danger:C.text;
