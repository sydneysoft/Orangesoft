...
export default function BlueprintPage(){
  ...
  const providerLabel=provider==="local"?"ИНКА / OLLAMA":"ЛОКАЛ / GROQ";
  const providerShort=provider==="local"?"ИНКА":"ЛОКАЛ";
  ...
  async function run(){
    ...
    if(!invalid){
      try{
        ...
        else{if(response.status===401)setAgent("AUTH REQUIRED");else if(response.status===409&&provider==="local")setAgent("LOCAL READ ONLY");else if(response.status===503)setAgent("SERVER SETUP");else setAgent("DISCONNECTED");output=data?.output||formatPlan(raw,plan);error=true}
  ...
  const metaColor=v=>v==="CONNECTED"||v==="SET"||v==="KEY SET"?C.ok:v==="LOCAL READ ONLY"?C.local:(String(v).includes("REQUIRED")||String(v).includes("MISSING")||v==="DISCONNECTED"||v==="SERVER SETUP")?C.danger:C.text;
