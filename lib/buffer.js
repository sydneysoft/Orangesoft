...
export async function createInstagramPost({ channelId, text, dueAt, imageUrl, altText }) {
  const data = await bufferGraphql(
  ...
  if (!result?.post) throw new Error("Buffer did not return the scheduled post");
  return result.post;
}
