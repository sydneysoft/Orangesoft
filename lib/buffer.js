...
function instagramPayload({ text, imageUrl, altText, mode, id }) {
  const input = {
    text,
...
export async function editInstagramPost(postId, { text, imageUrl, altText }) {
  const data = await bufferGraphql(
    `mutation EditInstagramPost($input: EditPostInput!) {
      editPost(input: $input) {
        ... on PostActionSuccess { post { id text dueAt sentAt status sharedNow shareMode externalLink } }
        ... on MutationError { message }
      }
    }`,
    {
      input: instagramPayload({
        id: postId,
        text,
        imageUrl,
        altText,
  ...
  if (!result?.post) throw new Error("Buffer did not return the edited post");
  return result.post;
}

export async function publishPostNow(postId, { text, imageUrl, altText }) {
  const data = await bufferGraphql(
...
export async function createInstagramPost({
  ...
  const input = instagramPayload({ text, imageUrl, altText, mode });
